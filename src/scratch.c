...
void assign_normals(){
    for (i32 i = 0; i < state.geo_buffer.used; i++){
        ...
        if (dot_product(normal, normal) != 0){
            if (state.geo_buffer.values[i].shade == 0){
...
void light_scene(){
    for (u32 i = 0; i < state.geo_buffer.used; i++){
        ...
        if (state.geo_buffer.values[i].shade == 0){
            ...
            // Convert face intensity value between 0-1
            vector3D fi = {r/255, g/255, b/255};

            // Light through each light source
            for (u32 j = 0; j < state.dir_light_buffer.used; j++){
                // Get light direction
                vector3D ld = rotate_against(state.dir_light_buffer.values[j].direction);

                // Get light intensity for face
                f32 dot = fmax(0, dot_product(ld, normal));
                vector3D li = multiply_by_scalar(state.dir_light_buffer.values[j].direction, dot);

                // Assign value to vertices
                vector3D li0 = multiply_by_scalar(state.dir_light_buffer.values[j].intensity, dot);
                state.vertex_buffer.values[state.geo_buffer.values[i].vi0].light_intensity.z += fi.x * li.x;
                state.vertex_buffer.values[state.geo_buffer.values[i].vi0].light_intensity.y += fi.y * li.y;
                state.vertex_buffer.values[state.geo_buffer.values[i].vi0].light_intensity.x += fi.z * li.z;

                vector3D li1 = multiply_by_scalar(state.dir_light_buffer.values[j].intensity, dot);
                state.vertex_buffer.values[state.geo_buffer.values[i].vi1].light_intensity.z += fi.x * li.x;
                state.vertex_buffer.values[state.geo_buffer.values[i].vi1].light_intensity.y += fi.y * li.y;
                state.vertex_buffer.values[state.geo_buffer.values[i].vi1].light_intensity.x += fi.z * li.z;

                vector3D li2 = multiply_by_scalar(state.dir_light_buffer.values[j].intensity, dot);
                state.vertex_buffer.values[state.geo_buffer.values[i].vi2].light_intensity.z += fi.x * li.x;
                state.vertex_buffer.values[state.geo_buffer.values[i].vi2].light_intensity.y += fi.y * li.y;
                state.vertex_buffer.values[state.geo_buffer.values[i].vi2].light_intensity.x += fi.z * li.z;
            }            
        }else if (state.geo_buffer.values[i].shade == 1){
            // Get vertex normals
            vector3D normal0, normal1, normal2;
            normal0 = state.vertex_buffer.values[state.geo_buffer.values[i].vi0].normal;
            normal1 = state.vertex_buffer.values[state.geo_buffer.values[i].vi1].normal;
            normal2 = state.vertex_buffer.values[state.geo_buffer.values[i].vi2].normal;
            u32 base_color = state.geo_buffer.values[i].color;
            ...
            for (u32 j = 0; j < state.dir_light_buffer.used; j++){
                ...
                // Get vertex light intensity,
                f32 dot0 = fmax(0, dot_product(ld, normal0));
                f32 dot1 = fmax(0, dot_product(ld, normal1));
                f32 dot2 = fmax(0, dot_product(ld, normal2));
...
739 lines refactored to: 

static void render_sprite3D(sprite3D sprite){
    vector3D zero = {0, 0, 0};
